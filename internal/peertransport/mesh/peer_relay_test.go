...
func TestPeerRelayDiscoveryDoesNotGrantApplicationAccess(t *testing.T) {
	...
	relay := &tailcfg.Node{ID: 1, Key: key.NewNode().Public(), DiscoKey: key.NewDisco().Public(), HomeDERP: 1, Addresses: []netip.Prefix{netip.MustParsePrefix("fd7a:115c:a1e0::77/128")}, AllowedIPs: []netip.Prefix{netip.MustParsePrefix("::/0")}}
