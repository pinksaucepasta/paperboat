...
func (p Plan) Validate() error {
	...
	if p.Canary.Path == "" || len(p.Canary.Path) > 256 || !strings.HasPrefix(p.Canary.Path, "/") || strings.ContainsAny(p.Canary.Path, "\x00\r\n") || p.Canary.ExpectedStatus < 100 || p.Canary.ExpectedStatus > 599 || p.Canary.TimeoutSeconds == 0 || p.Canary.TimeoutSeconds > 60 || p.Canary.Samples < 2 || p.Canary.Samples > 32 || !p.Canary.RequireEdge || !p.Canary.RequireConnector || !p.Canary.RequireRoute || !p.Canary.RequireOrigin {
		return ErrInvalidPolicy
	}
