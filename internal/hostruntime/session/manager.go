	...
	flush := func() error {
		for {
			...
			if m.config.Metrics != nil {
				...
				_ = m.config.Metrics.Record("paperboat_runtime_terminal_persistence_lag_bytes", float64(lag), nil)
			}
