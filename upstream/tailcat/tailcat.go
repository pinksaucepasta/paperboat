...
type Server struct {
	// TestOnlyPacketListener injects packet sockets for isolated path tests.
	TestOnlyPacketListener nettype.PacketListener
	...
	// LocalAddr is an authority-allocated address in fd7a:115c:a1e0::/48.
	// If unset, the address is derived from Key. Set before first use.
	...
	// AllowedPeers binds authorized node keys to allocated addresses. A non-nil
	// map enables authority mode; an empty map denies every peer. It requires
	// LocalAddr and cannot be combined with AllowedClients or forwarding.
	AllowedPeers map[key.NodePublic]netip.Addr
...
// Addr returns the server's configured or key-derived IPv6 address.
...
func (s *Server) DialAuthorizedUDP(ctx context.Context, peer key.NodePublic, disco key.DiscoPublic, ap netip.AddrPort) (ConnPacketConn, error) {
	if ctx == nil || s.lb == nil || peer.IsZero() || disco.IsZero() || !ap.IsValid() {
		return nil, errors.New("tailcat: invalid authorized UDP peer")
	}
	s.lb.mu.Lock()
...
func validateAllowedPeers(local netip.Addr, peers map[key.NodePublic]netip.Addr) error {
	if !validAuthorityAddr(local) {
		return errors.New("tailcat: invalid authority local address")
	}
	seen := map[netip.Addr]bool{local: true}
	for k, addr := range peers {
		if k.IsZero() || !validAuthorityAddr(addr) || seen[addr] {
			return errors.New("tailcat: invalid or duplicate authority peer address/key")
		}
		seen[addr] = true
	}
	return nil
...
func (s *Server) validateAuthority() error {
	if !s.LocalAddr.IsValid() && s.AllowedPeers == nil {
		return nil
	}
	if s.AllowedPeers == nil || len(s.AllowedClients) != 0 || s.AllowProxy != nil || s.OnTCPForward != nil || s.OnUDPForward != nil {
		return errors.New("tailcat: authority mode requires explicit peers and forbids proxy/legacy admission")
	}
...
func (s *Server) ReplaceAllowedPeers(peers map[key.NodePublic]netip.Addr) error {
	...
	b := s.lb
	if !b.authority {
		return errors.New("tailcat: server was not started in authority mode")
	}
	b.policyMu.Lock()
...
func (b *locoBackend) selfAllowedIPs() []netip.Prefix {
	if b.authority {
		return []netip.Prefix{b.addrPrefix}
	}
	return []netip.Prefix{b.addrPrefix, allIPv6}
}

func (b *locoBackend) serverAllowedIPs() []netip.Prefix {
	addr := b.serverAddr
	if !addr.IsValid() {
		addr = tcAddrForKey(b.serverPub)
	}
	if b.authority {
		return []netip.Prefix{pfxOf(addr)}
	}
	return []netip.Prefix{pfxOf(addr), allIPv6}
}
...
func (b *locoBackend) peerConfig(k key.NodePublic) (_ wgcfg.PeerConfig, ok bool) {
	...
	if !b.serverPub.IsZero() {
		...
		if k == b.serverPub {
			return withPSK(b.serverAllowedIPs()), true
		}
...
func (b *locoBackend) peerByIP(dst netip.Addr) (_ key.NodePublic, ok bool) {
	if !b.serverPub.IsZero() {
		// Authority clients can only route to the allocated server address.
		if b.authority && dst != b.serverAddr {
...
func (lb *locoBackend) Start() error {
	...
	if lb.serverPub.IsZero() {
		...
		nm.Peers = append(nm.Peers, (&tailcfg.Node{
			...
			AllowedIPs: lb.serverAllowedIPs(),
			HomeDERP:   derpRegion,
			Cap:        tailcfg.CurrentCapabilityVersion,
		}).View())
	}
	...
	if lb.serverPub.IsZero() && !lb.authority {
		// We're the server.
...
func (b *locoBackend) onMeow(src key.NodePublic, discoPub key.DiscoPublic) bool {
	...
	if (b.authority && !b.allowedPeers[src].IsValid()) || (b.allowedClients != nil && !b.allowedClients[src]) {
...
func (b *locoBackend) setPeerRelayNodes(nodes []*tailcfg.Node) error {
	if len(nodes) > 32 {
		return errors.New("tailcat: too many peer relay services")
	}
	keys := map[key.NodePublic]bool{}
	addresses := map[netip.Addr]bool{}
	copied := make([]*tailcfg.Node, 0, len(nodes))
	for i, n := range nodes {
		if n == nil || n.Key.IsZero() || n.DiscoKey.IsZero() || n.Key == b.pub || n.Key == b.serverPub || keys[n.Key] || n.HomeDERP == 0 || len(n.Addresses) != 1 || !n.Addresses[0].IsSingleIP() || !validAuthorityAddr(n.Addresses[0].Addr()) || n.Addresses[0].Addr() == b.addr || n.Addresses[0].Addr() == b.serverAddr || addresses[n.Addresses[0].Addr()] {
			return errors.New("tailcat: invalid peer relay service")
		}
		keys[n.Key] = true
...
type Client struct {
	...
	// ServerAddr is the authority-allocated server address. Set both this
	// and LocalAddr, or neither; authority mode permits no proxy routes.
	ServerAddr netip.Addr
...
func (c *Client) initLocked() error {
	...
	if c.LocalAddr.IsValid() || c.ServerAddr.IsValid() {
		if !validAuthorityAddr(c.LocalAddr) || !validAuthorityAddr(c.ServerAddr) || c.LocalAddr == c.ServerAddr {
			return errors.New("tailcat: invalid authority client addresses")
		}
	}
	lb := newLocoBackend(c.nodeKeyLocked(), ci.PresharedKey)
	lb.derpCarrierFactory = c.DERPCarrierFactory
	lb.testOnlyPacketListener = c.TestOnlyPacketListener
	lb.serverAddr = tcAddrForKey(ci.ServerPublic.NodePublic)
	if c.LocalAddr.IsValid() {
		lb.authority = true
		lb.addr, lb.addrPrefix = c.LocalAddr, pfxOf(c.LocalAddr)
		lb.serverAddr = c.ServerAddr
	}
	lb.logf = logf
	lb.dm = &tailcfg.DERPMap{}
	...
	lb.onDERPRecv = func(regionID tailcfg.DERPRegionID, src key.NodePublic, pkt []byte) bool {
		if c.OnRelayControl != nil && c.OnRelayControl(regionID, src, pkt) {
			return true
		}
		if !IsMeowPacket(pkt) {
			return false
	...
	ns.GetUDPHandlerForFlow = func(src, dst netip.AddrPort) (handler func(nettype.ConnPacketConn), intercept bool) {
		// Replies to open sockets are delivered before flow dispatch. Late
		// replies to closed ports must not create host-side UDP forwarding.
		return nil, true
	}
	lb.ns = ns
	sys.Set(ns)
	...
	localNets.AddPrefix(lb.addrPrefix)
	local, _ := localNets.IPSet()
	lb.setFilter(filter.New(lb.relayFilterMatches(), nil, local, tailcatULASet(), nil, logf))
	dialer.UseNetstackForIP = func(ip netip.Addr) bool {
		_, ok := lb.peerByIP(ip)
		return ok
	}
...
func (c *Client) ensureStarted(ctx context.Context) error {
	...
	c.lb.homeDERP = regions[0].RegionID
...
func (c *Client) ping(ctx context.Context) (PingResult, error) {
	...
	derpRegion := c.lb.peerDERPRegion(dstNode)
